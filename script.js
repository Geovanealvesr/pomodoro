...
// Acessa o campo input Pausa 
let pausa = document.getElementById('pausa')
// Acessa o campo input Sessões
let sessoes = document.getElementById('sessoes')
// Contador de segundos
let segundos

// Acesso dos audios de alarme nas variáveis
var bell = new Audio("./audio/bell.mp3")
var volta = new Audio("./audio/volta.mp3")
var final = new Audio("./audio/final.mp3")

// Acessa a tag audio/pause/play
var lofi = document.getElementById('lofi')
var pause = document.getElementById('pause')
var play = document.getElementById('play')

// Toca a musica e tira o botão pause e entra o play
function pausar(){
   lofi.pause()
...
// Pausa a musica e tira o botão play e coloca o pause
function executar(){
   lofi.play()
...
function iniciar() {
   ...
   // Verificação se os campos ação, pausa e sessão estão preenchidos
   if (acao.value == 0) {
      ...
      // Tocar a música 
      lofi.play()
      // Mostra o botão pause
      pause.style.setProperty('display', 'block', 'important')

      // Adicionar ao localStorage em forma de String os valores inseridos nos inputs ação/pause/sessões
      ...
      // Mostrar div timer, com o titulo e o relógio e quantidade de sessões
...
function momentoAcao() {
   ...
   // Pega o valor das sessões no localStorage e setar na variável sessoes_valor
   ...
   // Ativa a tag h3 id 'title'
   let title = document.getElementById('title')
   // Adiciona o valor 'AÇÃO' ao HTML
   title.innerHTML = "AÇÃO"
   // Muda o tamanho da fonte para 25pt
   title.style.fontSize = '25pt'
   // Muda a espessura do texto 
   title.style.fontWeight = 'bold'
   // Muda a cor do texto 
   title.style.setProperty('color', '#28a745', 'important')

   // Pega o valor de acao do localStorage, convertendo para Number e adiciona a min
   min = Number(localStorage.getItem('acao'))

   // O valor incia com  -1
   min = min - 1
   // Os segundos inicia com 59
   segundos = 59

   // Adiciona o valor de min ao h2 que tem a tag minutes_ok
   document.getElementById('minutes_ok').innerHTML = min
   // Adiciona o valor de segundos ao h2 que tem a tag seconds_ok
...
function momentoPausa() {
   ...
   // Função que será execultada a cada 60 segundos
   function minTimer() {
   ...
   function segTimer() {
      ...
      if (segundos <= 0) {
         ...
         if (min_pausa <= 0) {
            ...
            // Se acabar os minutos, Ele para o intervalo de min_interval e seg_interval e limpa
            clearInterval(min_interval)
            clearInterval(seg_interval)
            ...
            if (ses <= 0) {
               ...
               volta.play();
               // chama a função de momentoAcao reiniciando o ciclo
               momentoAcao()
            }
         }
         // Se os minutos não acabarem, adiciona mais 60 aos segundos e repete o ciclo.
         segundos = 60
      }
   }
